...
%% @doc    Integrationstests for the gossip2 and gossip_load modules.
%% @end
%% @version $Id$
-module(gossip2_SUITE).
...
%% @doc Sends a Msg to the gossip2 process
-spec send2gossip2(Msg::comm:message(), Delay::non_neg_integer()) -> reference().
send2gossip2(Msg, Delay) ->
    Group = pid_groups:group_with(gossip2),
    Pid = pid_groups:pid_of(Group, gossip2),
