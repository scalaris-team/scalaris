...
read(_Config) ->
    ...
    DB = ?DB:new(1),
    ?equals(?DB:read(DB, ?RT:hash_key("Unknown")), {ok, empty_val, -1}),
    ?DB:close(DB),
    ok.
...
write(_Config) ->
    ...
    ?equals(?DB:read(DB2, ?RT:hash_key("Key1")), {ok, "Value1", 1}),
    ?DB:close(DB),
    ok.
...
delete(_Config) ->
    ...
    ?equals(?DB:read(DB3, ?RT:hash_key("Key1")), {ok, empty_val, -1}),
...
get_version(_Config) ->
    erlang:put(instance_id, "db_SUITE.erl"),
    DB = ?DB:new(1),
    ?equals(?DB:get_version(DB, ?RT:hash_key("Key1")), failed),
    DB2 = ?DB:write(DB, ?RT:hash_key("Key1"), "Value1", 10),
    ?equals(?DB:get_version(DB2, ?RT:hash_key("Key1")), {ok, 10}),
    {DB3, ok} = ?DB:set_write_lock(DB2, ?RT:hash_key("Key2")),
    ?equals(?DB:get_version(DB3, ?RT:hash_key("Key2")), {ok, -1}),
    ?DB:close(DB),
    ok.
...
get_load_and_middle(_Config) ->
    erlang:put(instance_id, "db_SUITE.erl"),
    DB = ?DB:new(1),
    ?equals(?DB:get_load(DB), 0),
    DB2 = ?DB:write(DB, "Key1", "Value1", 1),
    ?equals(?DB:get_load(DB2), 1),
    DB3 = ?DB:write(DB2, "Key1", "Value1", 2),
    ?equals(?DB:get_load(DB3), 1),
    DB4 = ?DB:write(DB3, "Key2", "Value2", 1),
    ?equals(?DB:get_load(DB4), 2),
    ?equals(?DB:get_middle_key(DB4), failed),
    DB5 = ?DB:write(DB4, "Key3", "Value3", 1),
    DB6 = ?DB:write(DB5, "Key4", "Value4", 1),
    ?equals(?DB:get_middle_key(DB6), {ok, "Key2"}),
    OrigFullList = ?DB:get_data(DB6),
    {DB7, HisList} = ?DB:split_data(DB6, intervals:mk_from_node_ids("Key2", "Key4")),
    ?equals(?DB:read(DB7, "Key3"), {ok, "Value3", 1}),
    ?equals(?DB:read(DB7, "Key4"), {ok, "Value4", 1}),
    ?equals(?DB:get_load(DB7), 2),
    ?equals(length(HisList), 2),
    ?equals(length(?DB:get_data(DB7)), 2),
    DB8 = ?DB:add_data(DB7, HisList),
    ?equals(OrigFullList, ?DB:get_data(DB8)),
    ?DB:close(DB).
