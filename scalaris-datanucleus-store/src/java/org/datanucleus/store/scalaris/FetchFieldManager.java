...
public class FetchFieldManager extends AbstractFieldManager {
	...
	public Object fetchObjectField(int fieldNumber) {
		...
		String memberName = storeMgr.getNamingFactory().getColumnName(mmd,
				ColumnType.COLUMN);
		System.out.println("looking for field "+memberName +" "+acmd.getEntityName());
		if (result.isNull(memberName)) {
			return null;
		}
	...
	protected Object fetchObjectFieldInternal_RelationTypeNone(
		...
		if (mmd.getTypeConverterName() != null) {
			...
			Class datastoreType = TypeConverterHelper.getDatastoreTypeForTypeConverter(
					conv, mmd.getType());
...
//					.persistColumnAsNumeric(colmds != null && colmds.length>0? colmds[0] : null);
	...
	protected Object fetchObjectFieldInternal(AbstractMemberMetaData mmd,
		...
		if (relationType == RelationType.NONE) {
			...
			return IdentityUtils.getObjectFromPersistableIdentity(idStr, mmd.getAbstractClassMetaData(), ec);
			...
			if (mmd.hasCollection()) {
				...
				for (int i = 0; i < array.length(); i++) {
					...
					Object element = IdentityUtils.getObjectFromPersistableIdentity(idStr,
							elementCmd, ec);
					coll.add(element);
				}
				...
				while (keyIter.hasNext()) {
					...
					if (keyCmd != null) {
						...
						key = IdentityUtils.getObjectFromPersistableIdentity(idStr,
								keyCmd, ec);
					} else {
					...
					if (valCmd != null) {
						...
						val = IdentityUtils.getObjectFromPersistableIdentity(idStr,
								valCmd, ec);
					} else {
