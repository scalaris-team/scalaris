...
on({acceptor_accepted, {PaxosID, InRole}, Round, Value} = _Msg,
   {_RTMs, _TableName, MyRole, _LAcceptor, LLearner} = State) ->
    ?TRACE("tx_tm_rtm:on(~p)~n", [_Msg]),
...
on({tx_tm_rtm_commit, Client, ClientsID, TransLog},
   {RTMs, _TableName, _Role, _LAcceptor, LLearner} = State) ->
    ...
    GLLearner = cs_send:make_global(LLearner),
...
on({tx_tm_rtm_delete, TxId, Decision} = Msg,
   {RTMs, TableName, Role, LAcceptor, LLearner} = State) ->
