...
on({learner_decide, ItemId, _PaxosID, Value} = Msg, {_,_, Role} = State) ->
    ?TRACE("tx_tm_rtm:on(~p)~n", [_Msg]),
    ...
    case ok =/= ErrItem of
        true -> %% new / uninitialized
            ...
            %% created in init_RTM
            %% io:format("Holding back a learner decide for ~p~n", [Role]),
...
on({tx_tm_rtm_delete, TxId, Decision} = Msg, {RTMs, TableName, Role} = State) ->
...
%% footprint
on({tx_tm_rtm_delete_txid, TxId}, {_RTMs, TableName, _Role} = State) ->
...
%% footprint
on({tx_tm_rtm_delete_itemid, TxItemId}, {_RTMs, TableName, _Role} = State) ->
...
my_init_RTMs(TxState, ItemStates) ->
    ...
    RTMs = tx_state:get_rtms(TxState),
    [ cs_send:send(X, {tx_tm_rtm_init_RTM, TxState, ItemStates, Nth})
      || {_, X, Nth} <- RTMs ],
    ok.
...
my_get_tx_entry(Id, {_RTMS, TableName, _Role} = _State) ->
    case pdb:get(Id, TableName) of
        undefined -> {new, tx_state:new(Id)};
        Entry -> {tx_state:get_status(Entry), Entry}
    end.

my_get_item_entry(Id, {_RTMS, TableName, _Role} = _State) ->
    case pdb:get(Id, TableName) of
        ...
        Decision -> %% commit / abort
            ...
            of
                true ->
                    ...
                    cs_send:send_local(self(), {tx_tm_rtm_delete, TxId, Decision});
                false -> ok
            end
    end, ok.
