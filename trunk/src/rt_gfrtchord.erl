...
-record(rt_entry, {
        node :: node:node_type()
        , type :: entry_type()
        , group = other_dc :: other_dc | same_dc
...
maximum_entries() -> config:read(rt_gfrt_max_entries).
...
check_config() ->
    ...
    config:cfg_is_integer(rt_gfrtchord_max_entries) and
    config:cfg_is_greater_than(rt_gfrtchord_max_entries, 0)
    .
...
rt_entry_from(Node, Type, PredId, SuccId) ->
    #rt_entry{
        node=Node
        , type=Type
        %% if the hosts are the same, mark the entry as being in the same group
        , group = case comm:get_ip(node:pidX(Node)) == comm:get_ip(comm:this()) of
            true -> same_dc;
            false -> other_dc
        end
...
is_from_other_group(#rt_entry{group=same_dc}) -> false;
is_from_other_group(_) -> true.
