...
from \url{http://download.opensuse.org/repositories/home:/tschuett/} for
...
(ref. Section~\ref{sec.requirements})!
...
are shown in Section~\ref{sec.api.json}.
...
Requests are issued by sending a JSON object to this URL. The result will then
be returned as a JSON object. The following table shows how they both look
like:
...
\end{lstlisting}
  \item \code{write(Key, Value)} - write \code{Value} to \code{Key}, result:
\begin{lstlisting}[language=java]
{"status": "ok"} or
{"status": "fail", "reason": "timeout" or "abort"}
\end{lstlisting}
  \item \code{test_and_set(Key, OldValue, NewValue)} - atomic test-and-set (write \code{NewValue} to \code{Key} if the current value is \code{OldValue}), result:
\begin{lstlisting}[language=java]
{"status": "ok"} or
...
\end{lstlisting}
  ...
  \item \code{req_list(ReqList)} - process a list of requests, result:
\begin{lstlisting}[language=java]
[{"status": "ok"} or {"status": "ok", "value": <json_value>} or
 {"status": "fail", "reason": "timeout" or "abort" or "not_found"}]
\end{lstlisting}
  \item \code{req_list(TLog, ReqList)} - process a list of requests with a previous translog, result:
\begin{lstlisting}[language=java]
[{"status": "ok"} or {"status": "ok", "value": <json_value>} or
 {"status": "fail", "reason": "timeout" or "abort" or "not_found"}]
\end{lstlisting}
...
\end{lstlisting}
  ...
  \item \code{publish(Topic, Content)} - publish \code{Content} to \code{Topic}, result:
\begin{lstlisting}[language=java]
{"status": "ok"}
\end{lstlisting}
  \item \code{subscribe(Topic, URL)} - subscribe \code{URL} to \code{Topic}, result:
\begin{lstlisting}[language=java]
{"status": "ok"} or
{"status": "fail", "reason": "timeout" or "abort"}
\end{lstlisting}
  \item \code{unsubscribe(Topic, URL)} - unsubscribe \code{URL} from \code{Topic}, result:
\begin{lstlisting}[language=java]
{"status": "ok"} or
{"status": "fail", "reason": "timeout" or "abort" or "not_found"}
\end{lstlisting}
  \item \code{get_subscribers(Topic)} - get subscribers of \code{Topic}, result:
\begin{lstlisting}[language=java]
[<urls>]
\end{lstlisting}
