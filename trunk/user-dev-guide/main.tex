...
The main routines of our Chord node are in \url{src/dht\_node.erl} and
the join protocol is implemented in \url{src/dht\_node\_join.erl} (see
...
and the actual implementation is in \url{src/rt\_chord.erl}.
...
older one is in \url{src/transstore}. The newer one is more
modular. It provides an implementation of the paxos algorithm in
\url{src/paxos} and the transaction algorithms itself in
\url{src/transactions} (see also Chap.~\ref{chapter.transactions}).
...
The T-Man implementation can be found in \url{src/rm\_tman.erl} and
the Cyclon implementation in \url{src/cyclon}.
...
The implementation can be found in \url{src/vivaldi.erl}.
...
If its (future) successor is found and the \code{Id} is not the same, this
new node will send a \code{join_request} message including a reference to
itself and the chosen \code{Id}. 
If the chosen \code{Id} already exists, i.e. it is the successor's
\code{Id}, the node will try a new (randomly chosen) \code{Id}. 
Assuming the \code{join_request} message was send, it will be received by the
existing node in \code{dht_node.erl}

\codesnippet{dht_node.erl}{dht_node:join_message}{../src/dht_node.erl}

and trigger a call to \erlfun{dht\_node\_join}{process\_join\_msg}{/2} which
will set up a slide operation with the new node or deny the request if it is
not responsible for the key (anymore) any reply with a \code{}:
...
the join protocol. If everything is ok, i.e. \code{MyKey} is unique, it will
notify its ring maintenance process that it enters the ring, start all required
processes and join the slide operation in order to receive data from the
existing node.
...
\code{join_timeout} ms, the join will be re-started using
\erlfun{dht\_node\_join}{restart\_join}{/2}
\codesnippet{dht_node_join.erl}{dht_node_join:restart_join}{../src/dht_node_join.erl}
