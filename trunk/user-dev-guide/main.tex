...
(\code{cs_sup_or:init()} in this case) and then calls the start
...
The \code{cs_sup_or:init()} is finished and the supervisor module,
starts all the defined processes by calling the functions that were
defined in the list of the \code{cs_sup_or:init()}.
...
\code{Node}. This is intended and important, because \code{cs_node}
uses the database, but not vice versa. The supervisor first completely
initializes the DB process and afterwards calls
\code{cs_node:start_link}. We only go into details here, for the
latter.

\codesnippet{cs_node.erl}{cs_node:start_link}{../src/cs_node.erl}

\code{cs_node} implements the \code{gen_component} behaviour. This
...
The \code{gen_component} behaviour registers the \code{cs_node} in the
process dictionary. Formerly, the process had to do this himself, but
we moved this code into the behaviour. If the \code{cs_node} is the
...
\code{admin:add_nodes(1000)}, the boot-server would receive many join
...
\codesnippet{cs_join.erl}{cs_join:join1}{../src/cs_join.erl}
...
in \code{chordsharp.hrl}. For further details on the routing see
...
If a node joins an existing ring, \code{reliable_get_node} is called
for the own \code{Id} in \code{cs_join:join()}. This lookup delivers
the node who is currently responsible for the new node's identifier --
the successor for the joining node. If this lookup fails for some
reason, it is tried again, by recursivly calling the \code{join()}.
...
The \code{cs_node} notifies the ring maintenance, that he has a new
...
message in \code{cs_join:join_ring()}. The next steps after the
...
and the state of the \code{cs_node}.
...
\code{routingtable.erl} is a generic interface for routing tables. It
...
\code{rt_chord} implement the behaviour `routingtable'.

The macro ?RT is used to select the current implementation of routing
tables. It is defined in \code{chordsharp.hrl}.

\codesnippet{chordsharp.hrl}{chordsharp:rt}{../src/chordsharp.hrl}

The functions, that have to be implemented for a routing mechanism are
defined in the following file:

\codesnippet{routingtable.erl}{routingtable:behaviour}{../src/routingtable.erl}
...
\codesnippet{rt_chord.erl}{rt_chord:next_hop2}{../src/rt_chord.erl}
...
\code{cs_node}. At the destination the message is handled in
\code{cs_node.erl}:

\codesnippet{cs_node.erl}{cs_node:rt_get_node}{../src/cs_node.erl}
