...
If the entry exists, it is retrievd from the \code{gb_tree}.  If the id of
...
\todo{BUG: Instead of directly returning \code{Candidate} one should further
  ...
  check whether \code{Index} is zero, would become neccessary.}
...
perfom a lookup for the node with a \code{rt_get_node} message, including
...
\code{rt_get_node_response} message inclusing a reference to itself.
