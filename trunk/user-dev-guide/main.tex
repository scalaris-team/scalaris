...
\newcommand{\docversion}{0.3.0 draft}
...
management-server is optional and provides a global view on a \scalaris{}
deployment. On all servers, the \code{mgmt_server} configuration setting
defines the location of the management server. In the example, it is
...
The management server will maintain a list of nodes taking part in the
...
show you the number of nodes currently in the system. After a couple of
seconds a first \scalaris{} should have started and the number should
increase to one. The main page will also allow you to store and retrieve
key-value pairs but should not be used by applications to access
\scalaris{}. See Chapter~\sieheref{chapter.apis} for application APIs.

%The management server should show output similar to the following, when
...
%management server. When the third line is printed, it managed to contact the
%management server and joined the ring. In this case, it was the first node
%in the ring.
%\begin{lstlisting}
...
contact the management server and will join the ring by contacting the known
hosts. The number of nodes on the web page should have increased to two by
now.
...
This will add two further nodes to the deployment. The number passed to the
\code{./bin/joining_node.sh} script is the local id of the started node. The
web pages at \url{http://localhost:8000} should show the additional nodes.
...
nodes will look for the management server on \code{localhost} on port 14195. You
...
If you are using the default configuration on the management server it will
...
\code{./bin/joining_node.sh} and they will automatically contact the
...
\item openSUSE 11.0, 11.1, 11.2, 11.3, 11.4, Factory,
\item SLE 10, 11,
\item CentOS 5.4,
\item RHEL 5,
\item Debian 5.0 and
