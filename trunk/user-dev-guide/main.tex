...
also Chap.~\cite{chapter.join}). Our implementation of the routing
algorithms is described in more detail in Sect.~\ref{chapter.routing}
...
We have currently two generations of transaction algorithms. The
older one is in \code{src/transstore}. The newer one is more
modular. It provides an implementation of the paxos algorithm in
\code{src/paxos} and the transaction algorithms itself in
\code{src/transactions} (see also Chap.~\ref{chapter.transactions}).
...
information, e.g. for debugging. In \sieheref{chapter.join},
...
\erlfun{idholder}{get\_id}{}.
In case of a re-started node, this will be the last known ID. Otherwhise,
the \erlmodule{idholder} will return the ID (first) set by the
\code{\{\{idholder, id\}, KEY\}} option passed to \erlfun{idholder}{init}{/1}
or a random key. If the \code{dht_node}-process
fails and is re-started by its supervisor, this call to the idholder
ensures that the node still keeps its \code{Id}, assuming that the
idholder process is not failing and no passive load balancing takes place.
This is important in order not to destroy an existing load balance if a
node somehow fails.
%This is important for the
%load-balancing and for consistent responsibility of nodes to ensure consistent lookup in
...
the following 6 phases:
\begin{itemize}
  \item \makebox[4em][l]{\textbf{phase1}}
    getting the Id from the \erlmodule{idholder}
  ...
  \item \makebox[4em][l]{\textbf{phase3b}}
    waiting for the \erlmodule{idholder} to acknowledge an Id change
    (if applicable)
...
\medskip
{\centering\includegraphics{dht_node_join_phases2}}

\subsubsection{Phase 1}

Phase~1 is entered during the initialization of the \erlmodule{dht\_node}
process after requesting the ID from the \erlmodule{idholder}.
It receives its \code{idholder_get_id_response} message and contacts all
nodes set in the \code{known_hosts} configuration parameter. Their responses
are handled in phase~2.

\codesnippet{dht_node_join.erl}{dht_node_join:join_other_p1}{../src/dht_node_join.erl}

\subsubsection{Phase 2 and 2b}

Phase~2 collects all \erlmodule{dht\_node} processes inside the contacted
VMs. It therefor mainly processes \code{get_dht_nodes_response} messages and
integrates all received nodes into the list of \code{ContactNodes}.
...
use the ID from the \erlmodule{idholder}, issue a lookup for the responsible
...
\code{get_dht_nodes_response} messages arriving in phase~2b will be processed
the same way since no response for the request for the number of nodes to
sample has been processed yet.
For better efficiency, \code{get_dht_nodes_response} messages arriving in
any later phase will be processed anyway and the send \erlmodule{dht\_node}
processes will be integrated into the \code{ContactNodes}. These phases'
operations will however not be interrupted and nothing else is changed.
...
node will become the existing node's predecessor. The simulation will be
...
candidates by using the passive load balance algorithm and get the ID from
the best candidate. Note that during simulation at the existing node, this
ID may be different from the initial ID send during the lookup. If this ID
is the same as the one set at the \erlmodule{idholder}, we can continue
directly by sending a \code{join_request} message and going to phase~4. 
...
Otherwise we need to set the ID at the \erlmodule{idholder} first and wait
for its acknowledgement in phase~3b. A \code{join_request} is send once the
\code{idholder_get_id_response} message arrives.

\codesnippet{dht_node_join.erl}{dht_node_join:join_other_p3b}{../src/dht_node_join.erl}

The \code{join_request} message will be received by the existing node which
will set up a slide operation with the new node or deny the request if it is
not responsible for the key (anymore) and reply with a
...
The \code{join_timeout} parameter in the config files defines an overall
timeout for the whole join operation. If it takes longer than
\code{join_timeout} ms, the join will be re-started:

\erlfunindex{dht\_node\_join}{start\_over}
\codesnippet{dht_node_join.erl}{dht_node_join:start_over}{../src/dht_node_join.erl}

% TODO: write about all the other timeouts
...
\bibitem{t-man}
Mark Jelasity, Alberto Montresor, Özalp Babaoglu.
...
F. Schintke, A. Reinefeld, S. Haridi, T. Schütt.
