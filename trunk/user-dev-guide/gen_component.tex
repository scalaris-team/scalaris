\section{The \erlmodule{gen\_component}\erlmoduleindex{gen\_component}}
\svnrev{r832}
...
\subsection{A basic \erlmodule{gen\_component} including a message handler}
...
\erlmodule{gen\_component} and should return the initial state to be used for
this \erlmodule{gen\_component}.
...
\subsection{How to start a \erlmodule{gen\_component}?}
...
\begin{erlfunparams}
\erlparam{Module}{the the name of the module your component is
   implemented in}
...
\setlength{\parskip}{0pt}
\setlength{\itemsep}{0pt}
\item[\texttt{\{register, ProcessGroup, ProcessName\}}:] registers the new
  process with the given process group (also called instanceid) and name in
  the \erlmodule{process\_dictionary}.
\item[\texttt{\{register\_native, ProcessName\}}:] registers the process as
...
\subsection{When does a \erlmodule{gen\_component} terminate?}
...
The \erlmodule{process\_dictionary} (see
Section~\ref{sec:process_dictionary}) is a \erlmodule{gen\_component} which
registers itself as named Erlang process with the \erlmodule{gen\_component}
option \code{register_native} and therefore can be addressed by its name in
the Erlang shell. We send it a \code{\{no_message\}} and
\erlmodule{gen\_component} reports on the unhandled message. The
\erlmodule{process\_dictionary} itself continues to run and waits for
further messages.
...
\paragraph{Starting with non-default message handler.}
It is also possible to change the message handler right from the beginning
...
\subsection{Halting and pausing a \erlmodule{gen\_component}}

Using \erlfun{gen\_component}{kill}{Pid} and
\erlfun{gen\_component}{sleep}{Pid, Time} components can be terminated or
paused.

\subsection{Integration with \erlmodule{process\_dictionary}:
...
group of processes registered via the \erlmodule{process\_dictionary} (see
...
\subsection{Integration with \erlmodule{fd\_pinger}:
...
\subsection{The debugging interface of \erlmodule{gen\_component}:
...
\erlmodule{gen\_component}s. It supports breakpoints which can pause the
...
\item[\erlfun{gen\_component}{bp\_del}{Pid, BPName}:] The breakpoint
  ...
  \erlfun{gen\_component}{cont}{/1}. But the deleted breakpoint will no
...
\item[\erlfun{gen\_component}{bp\_barrier}{Pid}:]
  ...
  \code{ping} message and the set breakpoint may already be deleted before a
  ping arrives.
  ...
  This is where \erlfun{gen\_component}{bp\_barrier}{/1} can be used:
...
entering and continuing the breakpoint in the \erlmodule{gen\_component} can
happen independently later on, when the next \code{ping} message arrives.
...
\erlmodule{gen\_components} locally in the same Erlang virtual machine.
...
\subsection{Future use and planned extensions for \erlmodule{gen\_component}}
