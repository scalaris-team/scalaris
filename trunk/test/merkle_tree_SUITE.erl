...
tester_tree_hash(_) ->
    tester:test(?MODULE, prop_tree_hash, 3, 10, [{threads, 2}]).
...
tester_insert(_) ->
    tester:test(?MODULE, prop_insert, 3, 2, [{threads, 2}]).
...
tester_size(_) ->
  tester:test(?MODULE, prop_size, 3, 10, [{threads, 2}]).
...
tester_iter(_Config) ->
    ...
    tester:test(?MODULE, prop_iter, 3, 5, [{threads, 2}]).
...
tester_store_to_dot(_) ->
  tester:test(?MODULE, prop_store_to_dot, 3, 2, [{threads, 2}]).
