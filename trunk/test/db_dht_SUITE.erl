...
get_load_and_middle(_Config) ->
    ...
    DB2 = db_dht:write(DB, 1, ?VALUE("Value1"), 1),
    ?equals(db_dht:get_load(DB2), 1),
    DB3 = db_dht:write(DB2, 1, ?VALUE("Value1"), 2),
    ?equals(db_dht:get_load(DB3), 1),
    DB4 = db_dht:write(DB3, 2, ?VALUE("Value2"), 1),
    ?equals(db_dht:get_load(DB4), 2),
    DB5 = db_dht:write(DB4, 3, ?VALUE("Value3"), 1),
    DB6 = db_dht:write(DB5, 4, ?VALUE("Value4"), 1),
    OrigFullList = db_dht:get_data(DB6),
    {DB7, HisList} = db_dht:split_data(DB6, node:mk_interval_between_ids(2, 4)),
    ?equals(db_dht:read(DB7, 3), {ok, ?VALUE("Value3"), 1}),
    ?equals(db_dht:read(DB7, 4), {ok, ?VALUE("Value4"), 1}),
