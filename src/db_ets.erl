...
delete_entry_at_key(State = {DB, _Subscr}, Key) ->
    ets:delete(DB, Key),
...
get_chunk_({ETSDB, _Subscr} = DB, Interval, FilterFun, ValueFun, ChunkSize) ->
        ...
        _ ->
            ...
            case first_key_in_interval(ETSDB, Begin, Interval) of
                ...
                FirstKey ->
                    ...
                    FirstEntry = get_entry_(DB, FirstKey),
                    Chunk0 = case FilterFun(FirstEntry) of
                                 true -> [ValueFun(FirstEntry)];
                                 _    -> []
                             end,
                    {Next, Chunk} =
                        get_chunk_inner(DB, ets:next(ETSDB, FirstKey), FirstKey,
                                        Interval, FilterFun, ValueFun, ChunkSize - 1, Chunk0),
                    case Next of
                        '$end_of_table' ->
...
get_chunk_inner(_DB, RealStart, RealStart, _Interval, _FilterFun, _ValueFun, _ChunkSize, Chunk) ->
    %ct:pal("inner: 0: ~p", [RealStart]),
    % we hit the start element, i.e. our whole data set has been traversed
    {'$end_of_interval', Chunk};
get_chunk_inner(_DB, Current, _RealStart, _Interval, _FilterFun, _ValueFun, 0, Chunk) ->
    %ct:pal("inner: 1: ~p", [Current]),
    % we hit the chunk size limit
    {Current, Chunk};
get_chunk_inner({ETSDB, _Subscr} = DB, '$end_of_table', RealStart, Interval, FilterFun, ValueFun, ChunkSize, Chunk) ->
    %ct:pal("inner: 2: ~p", ['$end_of_table']),
    % reached end of table - start at beginning (may be a wrapping interval)
    get_chunk_inner(DB, ets:first(ETSDB), RealStart, Interval, FilterFun, ValueFun, ChunkSize, Chunk);
get_chunk_inner({ETSDB, _Subscr} = DB, Current, RealStart, Interval, FilterFun, ValueFun, ChunkSize, Chunk) ->
    %ct:pal("inner: 3: ~p", [Current]),
    case intervals:in(Current, Interval) of
        true ->
            Entry = get_entry_(DB, Current),
            Chunk1 = case FilterFun(Entry) of
                         true -> [ValueFun(Entry) | Chunk];
                         _    -> Chunk
                     end,
            Next = ets:next(ETSDB, Current),
            get_chunk_inner(DB, Next, RealStart, Interval, FilterFun, ValueFun, ChunkSize - 1, Chunk1);
        _ ->
            {'$end_of_interval', Chunk}
    end.
...
delete_chunk_(DB, Interval, ChunkSize) ->
    {Next, Chunk} = get_chunk_(DB, Interval, fun(_) -> true end, fun db_entry:get_key/1, ChunkSize),
    DB2 = lists:foldl(fun(Key, DB1) -> delete_entry_at_key(DB1, Key) end, DB, Chunk),
    {Next, DB2}.
