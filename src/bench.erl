...
read_read(ThreadsPerVM, Iterations) ->
    Msg = {bench_read_read, ThreadsPerVM, Iterations, comm:this()},
    manage_run(ThreadsPerVM, Iterations, [verbose], Msg).
...
manage_run(ThreadsPerVM, Iterations, Options, Message) ->
    ...
    Statistics = [receive
                      {done, X, WallClockTime, MinTime, AvgTime, MaxTime, Aborts} ->
                          io:format("BS: ~p @ ~p~n",[WallClockTime, X]),
                          {WallClockTime, MinTime, AvgTime, MaxTime, Aborts};
                      X -> io:format("~p~n", [X])
    ...
    case lists:member(verbose, Options) of
        true ->
            ...
            io:format("min. throughput: ~p (1/s)~n",
                      [[Iterations / element(4, Node) * 1000000.0 || Node <- Statistics]]),
            io:format("avg. throughput: ~p (1/s)~n",
                      [[Iterations / element(3, Node) * 1000000.0 || Node <- Statistics]]),
            io:format("max. throughput: ~p (1/s)~n",
                      [[Iterations / element(2, Node) * 1000000.0 || Node <- Statistics]]),
            ...
            io:format("aborts         : ~p~n",
                      [[element(5, Node) || Node <- Statistics]]);
