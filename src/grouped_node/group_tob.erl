...
-spec deliver(paxos_id(), proposal(), joined_state()) -> joined_state().
...
%      trigger next paxos
-spec deliver_current_decision(paxos_id(), proposal(), joined_state()) ->
    joined_state().
...
-spec postpone_future_decision(paxos_id(), proposal(), joined_state()) ->
    joined_state().
...
-spec deliver_postponed_decisions(joined_state()) -> joined_state().
...
-spec is_current_decision(paxos_id(), paxos_id()) -> boolean().
is_current_decision(NextPaxosId, PaxosId) ->
    PaxosId == NextPaxosId.

% @doc is this a future decision?
-spec is_future_decision(paxos_id(), paxos_id()) -> boolean().
...
-spec is_past_decision(paxos_id(), paxos_id()) -> boolean().
...
% @doc is this a previous decision?
-spec is_decision_from_other_group_id(paxos_id(), paxos_id()) -> boolean().
