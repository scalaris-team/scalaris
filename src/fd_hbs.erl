...
-type(rempid() :: %% locally existing subscriptions for remote pids
        {
          ...
          erlang:timestamp(),   %% delay remote demonitoring:
...
-type(state() :: {
       ...
       erlang:timestamp(),  %% local time of last pong arrival
       erlang:timestamp(),  %% remote is crashed if no pong arrives until this
...
-spec state_new(RemoteHBS::comm:mypid(), RemotePids::[rempid()],
                LastPong::erlang:timestamp(), CrashedAfter::erlang:timestamp(),
                LocalSubscriberTab::pdb:tableid(),
...
-spec state_get_last_pong(state())  -> erlang:timestamp().
state_get_last_pong(State)          -> element(3, State).
-spec state_set_last_pong(state(), erlang:timestamp()) -> state().
state_set_last_pong(State, Val)     -> setelement(3, State, Val).
-spec state_get_crashed_after(state()) -> erlang:timestamp().
state_get_crashed_after(State)      -> element(4, State).
-spec state_set_crashed_after(state(), erlang:timestamp()) -> state().
...
rempid_dec(Entry) ->
    TmpEntry = setelement(2, Entry, element(2, Entry) - 1),
    rempid_set_last_modified(TmpEntry, {0,0,0}).
-spec rempid_get_last_modified(rempid()) -> erlang:timestamp().
rempid_get_last_modified(Entry) -> element(3, Entry).
-spec rempid_set_last_modified(rempid(), erlang:timestamp()) -> rempid().
