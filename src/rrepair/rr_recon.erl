   ...
   State = #rr_recon_state{dest_rr_pid = DestRRPid, misc = Misc}) ->
    ...
    case RMethod of
        trivial ->
            ...
            {CKVTree, VSize} = decompress_kv_list(CKV, SigSize),
            ?DBG_ASSERT(Misc =:= []),
        ...
        shash ->
            ...
            fd:subscribe(self(), [DestRRPid]),
            % convert db_chunk to a map for faster access checks
            {SHTree, 0} = decompress_kv_list({SH, <<>>}, SigSize),
    ...
    if not SyncFinished ->
       ...
       true ->
           ...
           ReqIdx = lists:usort([Idx || {_Version, Idx} <- mymaps:values(OtherDBChunk1)]
                                    ++ ToReqIdx),
           ToReq2 = compress_idx_list(ReqIdx, ChunkSize - 1, [], 0, 0),
           NewStats2 =
   ...
   State = #rr_recon_state{stage = reconciliation,    initiator = true,
    ...
    if not SyncFinished ->
       ...
       true ->
           ...
           ReqIdx = lists:usort([Idx || {_Version, Idx} <- mymaps:values(OtherDBChunk1)]),
           OtherDiffIdx = compress_idx_list(ReqIdx, ChunkSize - 1, [], 0, 0),
...
%%              erlang:byte_size(erlang:term_to_binary(MyDiffIdx, [compressed]))]),
    ...
    {DBChunkTree, VSize} = decompress_kv_list(OtherDBChunk, SigSize),
...
on({resolve_req, DBChunk, DupesCount, Fr_p1_real, SigSize, _DestReconPid} = _Msg,
   State = #rr_recon_state{stage = resolve,           initiator = true,
    ...
    {DBChunkTree, VSize} = decompress_kv_list(DBChunk, SigSize),
    OrigDBChunkLen = mymaps:size(DBChunkTree),
    % worst case diff here is 100% since both nodes now operate on the
    % differences only and each node may have what the other node does not
    Fr_p2_real = trivial_worst_case_failrate(
   ...
   State = #rr_recon_state{stage = reconciliation,    initiator = false,
    ...
    if RTree =:= [] ->
       ...
       true ->
           ...
           NewState#rr_recon_state{misc = [{fail_rate_target_per_node, FRPerNode},
                                           {fail_rate_target_phase1, FR_p1},
...
begin_sync(State = #rr_recon_state{method = merkle_tree, params = Params, initiator = true,
    ...
    Stats1 = rr_recon_stats:set([{tree_size, MTSize}], Stats),
    #merkle_params{fail_rate = FRTotal, num_trees = NumTrees,
...
compress_kv_list([_ | _] = KVList, {AccDiff, AccV}, SigSize, VSize, KeyComprFun) ->
    ...
    {<<AccDiff/bitstring, DiffKBin/bitstring>>, DiffVBin, KV0List, Dupes};
...
decompress_kv_list({<<>>, <<>>}, _SigSize) ->
    {mymaps:new(), 0};
decompress_kv_list({KeyDiff, VBin}, SigSize) ->
...
merkle_next_signature_sizes(
  ...
  MyMaxItemsCount, OtherMaxItemsCount) ->
    ...
    if MyMaxItemsCount =/= 0 andalso OtherMaxItemsCount =/= 0 ->
           AffectedItemsI = calc_max_different_items_total(
    ...
    AffectedItemsL = calc_max_different_items_total(
...
merkle_resolve_add_leaf_hashes([{OtherMaxItemsCount, Bucket} | Rest],
    ...
    ExpDelta = Params#merkle_params.exp_delta, % TODO: we must not assume a uniform distribution here!
...
%%             [self(), NumRestLeaves, {SigSize, VSize}, FR_next, ThisFR, PrevFR, NextFR]),
    ...
    if BucketSize > 0 ->
           ...
           {OBucketTree, VSize} = decompress_kv_list({OBucketKBin, OBucketVBin}, SigSize),
           ok;
       true ->
...
%%             [ReqIdx, ToResolveIdx, Params#merkle_params.bucket_size]),
    ...
    merkle_resolve_retrieve_leaf_hashes(
      Rest, NHashesK, NHashesV, NDupesCount, FRAllLeaves, NumRestLeaves - 1, NextFR,
...
merkle_resolve_retrieve_leaf_hashes(
  [], HashesK, HashesV, DupesCount, _FRAllLeaves, _NumRestLeaves, _PrevFR,
...
-spec merkle_resolve_leaves_receive(
        State::state(), {HashesK::bitstring(), HashesV::bitstring(),
                         DupesCount::bitstring()}) -> NewState::state().
