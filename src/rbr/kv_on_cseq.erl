...
-export([commit_read/3, commit_read_feeder/3]).
...
-type readlock()  :: [?RT:key()]. %% non_neg_integer(). %% later: [tx_id_keys].
-type writelock() :: ?RT:key() | false. %% later: tx_id_key | write_lock_is_free
...
write(Key, Value) ->
    ...
    receive
        ?SCALARIS_RECV({qwrite_done, _ReqId, _Round, Value}, {ok}); %%;
        ?SCALARIS_RECV({qwrite_deny, _ReqId, _Round, _Value, Reason},
...
-spec cc_set_rl(no_value_yet | {writelock(), version()}, prbr:write_filter(),
                {txid(), version()}) ->
                       {boolean(), none}.
cc_set_rl(no_value_yet, _WF, _Val = {_TxId, TLogVers}) ->
    {true, none};
...
-spec wf_set_wl
       (prbr_bottom, UI :: none, {txid(), 0}) -> db_entry();
       (db_entry(), UI :: none, {txid(), version()}) -> db_entry().
...
-spec commit_read_feeder(tx_tlog:tlog_entry(), ?RT:key(), ok) ->
                                {tx_tlog:tlog_entry(), ?RT:key(), comm:erl_local_pid()}.
commit_read_feeder(TLogEntry, TxId, ok) ->
    {TLogEntry, TxId, self()}.

-spec commit_read(tx_tlog:tlog_entry(), ?RT:key(), comm:erl_local_pid()) -> ok.
commit_read(TLogEntry, TxId, ReplyTo) ->
    Key = tx_tlog:get_entry_key(TLogEntry),
    HashedKey = case is_list(Key) of
                    true -> ?RT:hash_key(Key);
                    false -> Key
...
-spec cc_commit_read(no_value_yet | {readlock, version()}, prbr:write_filter(),
                {txid(), version()}) ->
                       {boolean(), none}.
cc_commit_read(no_value_yet, _WF, _Val = {TxId, TLogVers}) ->
    {true, none};
cc_commit_read({RL, Vers}, _WF, _Val = {TxId, TLogVers}) ->
    {TLogVers =:= Vers, none}.

-spec wf_unset_rl
       (prbr_bottom, UI :: none, {txid(), 0}) -> prbr_bottom;
       (db_entry(), UI :: none, {txid(), version()}) -> db_entry().
wf_unset_rl(prbr_bottom, _UI = none, {TxId, _Vers}) -> prbr_bottom;
...
%% -spec rf_wl_rl_vers(db_entry() | prbr_bottom) ->
%%                            {readlock(), version()} | no_value_yet.
%% rf_wl_vers(prbr_bottom) -> no_value_yet;
%% rf_wl_vers(X) ->           {readlock(X), vers(X)}.
%% 
%% -spec cc_commit_read(no_value_yet | {readlock, version()}, prbr:write_filter(),
%%                 {txid(), version()}) ->
%%                        {boolean(), none}.
