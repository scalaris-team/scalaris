...
also Chap.~\ref{chapter.join}). Our implementation of the routing
...
operations will not be interrupted and nothing else is changed.
...
\medskip
{\small
\begin{tabular}{lP{2.4cm}P{2.75cm}P{2.35cm}P{3.15cm}M{1.85cm}}
  \toprule
  ...
  & timeouts $< 3$?\footnote{set by the \code{join_request_timeouts} config parameter}\newline
    $\rightarrow$ contact candidate\newline
    otherwise:\newline
    remove candidate\newline
    no candidates left?\newline
    $\rightarrow$ phase~2 or 2b\newline
    otherwise:\newline
    $\rightarrow$ contact next one\newline
    $\rightarrow$ phase~3b or 4
  & \tn
  \bottomrule
\end{tabular}
}
