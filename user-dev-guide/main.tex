...
configured in \code{bin/scalaris.cfg}. The default value is
{\tt error}; only errors and severe problems are logged.
...
\begin{itemize}
\item use \erlfun{ets}{i} to get details on the local state of some processes
...
\item Joining a process group (InstanceId is a group name)
...
\item \code{init_stabilize/3} is called periodically to rebuild the
  ...
  successor and the old (internal) routing table state.
...
\erlfunindex{rt\_beh}{get\_keys\_for\_replicas}
\item \code{get_keys_for_replicas/1} Returns for a given (un-hashed)
...
\erlfunindex{rt\_beh}{update\_pred\_succ\_in\_dht\_node}
\item \code{update_pred_succ_in_dht_node/3} update predecessor and successor
  node stored in an external routing table state.
...
\erlfunindex{rt\_beh}{check\_fd}
\item \code{check/5}, \code{check/6}, \code{check_fd/2} are implemented in
...
\erlfunindex{rt\_simple}{get\_keys\_for\_replicas}
\codesnippet{rt_simple.erl}{rt_simple:get_keys_for_replicas}{../src/rt_simple.erl}
This \code{get_keys_for_replicas/1} implements symmetric replication.
...
\erlfunindex{rt\_simple}{update\_pred\_succ\_in\_dht\_node}
\codesnippet{rt_simple.erl}{rt_simple:update_pred_succ_in_dht_node}{../src/rt_simple.erl}
Updates the external routing table state with the given successor.
...
\erlfun{rt\_chord}{get\_keys\_for\_replicas}{/1} and
...
the detailed startup logs. See Section~\sieheref{sec:logging}}.
...
function \erlfun{sup\_dht\_node}{start\_link} is called by the Erlang
...
(\erlfun{sup\_dht\_node}{init} in this case) and then calls the start
function (\code{start_link} here.

So, lets have a look at \erlfun{sup\_dht\_node}{init}, the 'Scalaris
\emph{or} supervisor'.
...
\code{idholder:get_id()}. In the first call, a random identifier is
...
If its (future) successor is found, this new node will send a \code{join}
...
This triggers a call to \erlfun{dht\_node\_join}{join\_request}{/2} on the
old node.
...
%   \erlfun{dht_node}{start}. Nevertheless, they could be implemented as
...
\begin{tabular}{|r|p{11.5cm}|}
 ...
 \code{docroot} & root directory of the boot server's webserver \\
